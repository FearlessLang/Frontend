...
public record Gamma(Gamma tail, String name, T t, Change current){
  ...
  public Gamma filterFTV(List<B> bs){//we only care about dom(bs)
    ...
    if (this == _empty){ return this; }
    var rest= tail.filterFTV(bs);
    var v= current.view();
    if (v.isEmpty()){ return new Gamma(rest, name, t, current); }
    if (!hasOnlyFTV(v.get(),bs)){ return new Gamma(rest, name, t, new Drop(TypeSystemErrors.filterFTVWhy(v.get(), bs))); }
