...
public record ViewPointAdaptation(Kinding k){
  ...
  private Change of(T t, List<B> delta, Literal l, M m){    //Literal l, M m, T atDrop
    RC rc0= l.rc();
    RC rc=  m.sig().rc();
    if (discard(t, delta, rc0)){ return Change.drop(TypeSystemErrors.discardWhy(l,m,t)); }//TODO: it was , delta, rc0,kindIsoImm(t, delta) and we may need it again for good errors
