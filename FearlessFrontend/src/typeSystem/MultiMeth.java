...
class MultiMeth{
  ...
  public static List<MType> of(List<B> d, MType mType){
    ...
    add(out,mType);
    add(out,apply("Strong",d,mType,Mode.strong,F.glb,Mode.strong,F.lub));
    add(out,apply("Flexy",d,mType,Mode.flexy,F.glb,Mode.flexy,F.lub));
    add(out,apply("UseReadH",d,mType,Mode.useRead,F.glb,Mode.hyg,F.lub));
  ...
  private static void oneMutHToMut(LinkedHashMap<Key,MType> out, List<B> d, MType m){
    ...
    add(out,new MType("UseMutH+Rec", Mode.hyg.of(m.rc()), tsi, t));
  ...
  private static void iMutHToMut(LinkedHashMap<Key,MType> out, List<B> d, MType m, int i){
    ...
    var t= modeF(d,m.t(), Mode.hyg, F.lub);
    add(out,new MType("UseMutH+"+i, Mode.flexy.of(m.rc()), tsi, t));
  }
