...
public record Kinding(TypeSystemErrors err){
  ...
  public void check(E toErr, KindingTarget target, int index, List<B> bs, T t, EnumSet<RC> allowed){ switch(t){
    ...
    case T.RCX rcx -> checkRCX(toErr, target, index, bs, rcx, allowed);
  ...
  private void checkRCC(E toErr, KindingTarget target, int index, List<B> bs, T.RCC rcc, EnumSet<RC> allowed){
    ...
    for (int i= 0; i < params.size(); i++){ check(toErr, rcc, i, bs, args.get(i), EnumSet.copyOf(params.get(i).rcs())); }
  }
  private void checkRCX(E toErr, KindingTarget target, int index, List<B> bs, T.RCX rcx, EnumSet<RC> allowed){
    if (!ofRCX(bs, rcx,allowed)){ throw err.typeNotWellKinded(toErr,target,index,allowed); }
  }
...
public record Kinding(Function<TName,Literal> decs, TypeSystemErrors err){
  ...
  public void check(E toErr, List<B> bs, T t, EnumSet<RC> allowed){
    ...
    throw err.typeNotWellKinded(toErr,(T.RCC)t);
  }
  public boolean of(List<B> bs, T t, EnumSet<RC> allowed){ return switch(t){
    case T.RCC rcc -> ofRCC(bs, rcc, allowed);
    case T.RCX rcx -> ofRCX(bs, rcx, allowed);
