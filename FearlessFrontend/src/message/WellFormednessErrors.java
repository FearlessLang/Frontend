...
public record WellFormednessErrors(String pkgName) {
	...
	public static class ErrToFetchContext extends RuntimeException {
		public ErrToFetchContext(IT.RCC c) {
			this.c = c;
		}
	...
	Err err() {
		return new Err(y -> y, x -> x, trunk -> new CompactPrinter(pkgName, Map.of(), trunk), new StringBuilder());
	}

	public FearlessException notClean(Ref uri, FileFull f) {
		var e = err().line("Package directives outside of rank file.")
				.line("Only the rank file should not contain directives like maps and uses.").blank()
				.line("Found non-empty:");
		boolean any = false;
		if (!f.maps().isEmpty()) {
			e.bullet("maps: " + previewList(f.maps(), 5));
			any = true;
		}
		if (!f.uses().isEmpty()) {
			e.bullet("uses: " + previewList(f.uses(), 8));
			any = true;
		}
		assert any;
	...
	public FearlessException expectedSingleUriForPackage(List<Ref> heads, String pkgName) {
		if (heads.isEmpty()) {
			return badRank(err().line("No rank file found for package " + Err.disp(pkgName) + ".")
					.line("Each package must have exactly one source file whose name is their rank.")
					.line("in some folder inside the project folder.")).wf();
		}
		var e = err().line("Ambiguous rank file for package " + Err.disp(pkgName) + ".")
	...
	public Err badRank(Err err) {
		return err.line(
				...
				.line("The rank file is the file whose name matches the rank name.").blank()
	...
	public FearlessException usedDeclaredNameClash(String pkgName, Set<TName> names, Set<String> keySet) {
		for (TName n : names) {
			if (!keySet.contains(n.s())) {
				continue;
			}
			return err()
					.line("Name clash: name " + Err.disp(n.s()) + " is declared in package " + Err.disp(pkgName) + ".")
					.line("Name " + Err.disp(n.s()) + " is also used in a \"use\" directive.").wf()
	...
	public FearlessException usedUndeclaredName(TName tn, String contextPkg, List<TName> scope, List<TName> all) {
		return new UndeclaredNameContext(this::err, tn, contextPkg, scope, all,
				all.stream().map(TName::pkgName).filter(p -> !p.isEmpty()).distinct().sorted().toList(), tn.pkgName(),
				tn.simpleName()).build();
	}

	private record UndeclaredNameContext(Supplier<Err> err, TName tn, String contextPkg, List<TName> scope,
			List<TName> all, List<String> allPkgs, String typedPkg, String typedSimple) {
		FearlessException build() {
			return pkgDoesNotExist().or(this::otherArity).orElseGet(this::undeclaredInPkg);
		}
		...
		private <A, R> List<R> userMap(Function<A, R> f, Stream<A> s) {
			return s.map(f).distinct().sorted().toList();
		}
		...
		private Optional<FearlessException> otherArity() {
			...
			var e = err.get()
					.line("Name " + Err.disp(typedSimple) + " is not declared with " + tn.arity()
							+ " type parameter(s) in package " + Err.disp(targetPkg) + ".")
					.line("Name " + Err.disp(typedSimple) + " is only declared with "
							+ (arities.size() == 1 ? arities.getFirst() + " type parameter(s)."
		...
		private List<String> simpleNames(List<TName> xs) {
			return userMap(TName::simpleName, xs.stream());
		}

		private void addOtherPkgNotePkgExplicit(Err e) {
			var sameSimpleOther = userMap(TName::s, all.stream().filter(t -> !t.pkgName().equals(typedPkg))
		...
		private void addOtherPkgNotePkgImplicit(Err e) {
			...
			NameSuggester.bestName(typedSimple, simpleCandidates).ifPresent(bestSimple -> addOptionsList(
					userMap(TName::s, all.stream().filter(t -> t.simpleName().equals(bestSimple))), e));
		}
		...
		void addOptionsList(List<String> ss, Err e) {
			...
			e.line(Join.of(ss.stream().map(Err::disp), "Did you mean ", " or ", " ?")).line(addUse);
		}
		...
		private Span at() {
			return Parser.span(tn.pos(), tn.s().length());
		}
	}
	...
	public FearlessException unknownUseHead(TName tn) {
		...
		return err()
				...
				.wf().addFrame("package header", at);
	}

	public FearlessException genericTypeVariableShadowTName(String pkgName, Map<TName, Set<X>> allXs,
			List<String> allNames, Set<String> use) {
		...
		for (var n : mergeAllXs) {
			...
			if (clashDec || clashUse) {
				return shadowMsg(pkgName, n, clashUse);
			}
		}
		throw Bug.unreachable();
	}
	...
	private FearlessException shadowMsg(String pkgName, T.X n, boolean use) {
		return err().line("Type parameter " + Err.disp(n.name()) + " is declared in package " + Err.disp(pkgName) + ".")
				.line("Name " + Err.disp(n.name()) + " is also used "
						+ (use ? "in a \"use\" directive." : "as a type name."))
				.wf().addFrame("a type name", n.span().inner);
	}

	public FearlessException duplicatedBound(List<RC> es, T.X n) {
		RC dup = es.stream().filter(e -> es.stream().filter(ei -> ei.equals(e)).count() > 1).findFirst().get();
		return err().line("Duplicate reference capability in the type parameter " + Err.disp(n.name()) + ".")
				.line("Reference capability " + Err.disp(dup.name()) + " is repeated.").wf().addSpan(n.span().inner);
	}

	public FearlessException duplicatedName(TName name) {
		return err().line("Duplicate type declaration for " + err().tNameADisp(name) + ".").wf().addFrame("a type name",
				Parser.span(name.pos(), name.s().length()));
	}

	public FearlessException circularImplements(Map<TName, E.Literal> rem) {
		TName name = findCycleNode(rem);
		return err().line("Circular implementation relation found involving " + err().tNameADisp(name) + ".").wf()
	...
	public FearlessException noSourceToInferFrom(E.Literal origin, M m) {
		...
		if (empty) {
			return err().line("Cannot infer signature and name for a method with " + size + " parameters.")
					.line("No supertype has a method with " + size + " parameters.").wf().addSpan(m.sig().span().inner)
		...
		if (argsPresentNoTypes) {
			return err().line("Cannot infer signature of method " + name + ".")
					.line("No supertype has a method named " + name + " with " + size + " parameters.").wf()
					.addSpan(m.sig().span().inner).addFrame(err().expRepr(origin), origin.span().inner);
		}
		var e = err().line("Missing return type for method " + name + ".")
				.line("Add an explicit return type before '->'.");
		if (allParHasType) {
			e = e.line("Alternatively (less common), if you intended to override and omit the signature,")
					.line("the signature must be inherited from a supertype.");
		} else { // size == 0
			e = e.line("If you intended to override and omit the signature,")
					.line("the signature must be inherited from a supertype.");
		}
		return e.line("Cannot infer signature of method " + name + ".")
				.line("No supertype has a method named " + name + " with " + size + " parameters.").wf()
				.addSpan(m.sig().span().inner).addFrame(err().expRepr(origin), origin.span().inner);
	}

	public String refCapDisagreement() {
		return "Reference capability disagreement";
	}

	public String retTypeDisagreement() {
		return "Return type disagreement";
	}

	public String argTypeDisagreement(int i) {
		return "Type disagreement about argument " + i;
	}
	...
	public FearlessException noAgreement(Agreement at, FreshPrefix fresh, List<?> res, String msg) {
		...
		var e = err().line(
				msg + " for method " + err().methodSig(rc, at.mName()) + " with " + at.mName().arity() + " parameters.")
				.line(Join.of(res.stream().map(o -> {// Can be RC or inference.IT.RCC
					if (o instanceof inference.IT.RCC rcc) {
						return err().typeRepr(rcc);
					}
					return Err.disp(o);
				}), "Different options are present in the implemented types: ", ", ", "."))
				.line(Err.up(err().expRepr(at.lit())) + " must declare a method " + err().methodSig(at.mName())
						+ " explicitly choosing the desired option.");
		return e.wf().addFrame(err().expRepr(at.lit()), at.span());
	}

	public FearlessException methodGenericArityDisagreementBetweenSupers(Agreement at, FreshPrefix fresh,
			List<List<B>> res) {
		var e = err()
				.line("The number of type parameters disagrees for method " + err().methodSig(at.mName()) + " with "
						+ at.mName().arity() + " parameters.")
	...
	public FearlessException methodGenericArityDisagreesWithSupers(Agreement at, FreshPrefix fresh, int userArity,
			int superArity, List<B> userBs, List<B> superBs) {
		String sB = Err.disp(superBs.stream().map(b -> new B("-", b.rcs())).toList());
		return err()
				.line("Invalid method implementation for "
						+ err().methodSig(at.rc().orElse(RC.imm).toStrSpace(), at.lit(), at.mName()) + ".")
				.line("The method " + err().methodSig(at.mName()) + " declares " + userArity
						+ " type parameter(s), but supertypes declare " + superArity + ".")
				.line("Local declaration: " + Err.disp(userBs) + ".").line("From supertypes: " + sB + ".")
				.line("Change the local number of type parameters to " + superArity + ", or adjust the supertypes.")
				.wf().addFrame(err().expRepr(at.lit()), at.span());
	}
	...
	public FearlessException methodBsDisagreementBetweenSupers(Agreement at, FreshPrefix fresh, List<List<B>> res) {
		...
		return err()
				...
				.line("Make the supertypes agree on these bounds, or remove one of the conflicting supertypes.").wf()
				.addFrame(err().expRepr(at.lit()), at.span());
	}

	public FearlessException methodBsDisagreesWithSupers(Agreement at, FreshPrefix fresh, List<B> userBs,
			List<B> superBs) {
		...
		return err()
				.line("Invalid method implementation for "
						+ err().methodSig(at.rc().orElse(RC.imm).toStrSpace(), at.lit(), at.mName()) + ".")
				.line("The local declaration uses different capability bounds than the supertypes for type parameter "
						+ (i + 1) + " of " + m + ".")
				.line("Local: " + uB + ".").line("From supertypes: " + sB + ".")
				.line("The parameter name may differ; only the position matters.")
				.line("Change the local bounds to match the supertypes, or adjust the supertypes.").wf()
				.addFrame(err().expRepr(at.lit()), at.span());
	}

	private int firstRcsDisagreementIndex(List<List<B>> res) {
		return IntStream.range(0, res.getFirst().size()).filter(i -> {
	...
	public FearlessException itTooDeep(E at, IT.RCC blame) {
		return err().line("Type " + err().typeRepr(blame)).line("grew incontrollably during inference.").wf()
				.addFrame(err().expRepr(at), at.span().inner);
	}

	public FearlessException ambiguousImpl(E.Literal origin, FreshPrefix fresh, boolean abs, M m,
			List<inference.M.Sig> options) {
		return err().line("Cannot infer the name for a method with " + m.sig().ts().size() + " parameters.")
				.line("Many" + (abs ? " abstract" : "") + " methods with " + m.sig().ts().size()
						+ " parameters could be selected:")
				.line(Join.of(options.stream().map(mi -> Err.disp(mi.rc().get() + " " + mi.m().get().s())),
						"Candidates: ", ", ", "."))
				.wf().addSpan(m.sig().span().inner).addFrame(err().expRepr(origin), origin.span().inner);
	}

	public FearlessException ambiguousImplementationFor(List<M.Sig> ss, List<TName> options, Agreement at,
			FreshPrefix fresh) {
		return err()
				.line("Ambiguous implementation for method " + Err.disp(at.mName().s()) + " with " + at.mName().arity()
						+ " parameters.")
	...
	public FearlessException multipleWidenTo(E.Literal owner, List<IT.C> widen) {
		var e = err().line(err().expRepr(owner) + " implements \"base.WidenTo[_]\" more than once.").line(
				"At most one \"base.WidenTo[_]\" supertype is allowed, because it defines the preferred widened type.")
				.blank().line("Found the following base.WidenTo supertypes:");
	...
	public FearlessException duplicatedNamedLiteral(E.Literal owner, M m, E.Literal in) {
		...
		return err().line(ctx + " implements method " + err().methodSig(m.sig().m().get()) + ".")
				.line("The body of method " + err().methodSig("", owner, m.sig().m().get())
						+ " needs to be duplicated to satify multiple RC overloads from the supertypes.")
				.line("However, it contains " + err().expRepr(in) + ".")
				.line("Object literals with their own unique explicit type can not be duplicated.").wf()
				.addFrame(err().expRepr(owner), owner.span().inner);

		// assert l.infName() : "TODO: to user facing error. Duplicating body contains
		// user-named literal: "+l.name();
	}
	...
	public FearlessException extendedSealed(E.Literal owner, FreshPrefix fresh, TName isSealed) {
		...
		String ctx = Err.up(err().expRepr(owner));
		return err().line(ctx + " implements sealed type " + err().tNameADisp(isSealed) + ".")
	...
	public FearlessException intLiteralOutOfRange(TName lit) {
		...
		return err().line("Integer literal is out of range for \"base.Int\".")
				...
				.line("Hint: if you need arbitrary precision numbers, use \"base.Num\".").wf()
				.addSpan(lit.approxSpan().inner);
	}

	public FearlessException natLiteralOutOfRange(TName lit) {
		BigInteger v = LiteralDeclarations.natLiteralBig(lit.simpleName());
		return err().line("Natural literal is out of range for \"base.Nat\".")
	...
	public FearlessException floatLiteralNotExactlyRepresentable(TName lit) {
		...
		var e = err().line("Float literal is not exactly representable as \"base.Float\".")
				.line("\"base.Float\" must be representable exactly as a 64-bit IEEE 754 double.")
				.line("This literal is: " + raw + ".");
		if (!Double.isFinite(d)) {
			e.line("This literal overflows; the nearest representable value is " + Err.disp(near) + ".");
		} else {
