...
A: a b c } f e
""");}
@Test void err_unopened_recover_top(){fail("""
...
OrderHash[T,E:*]:Order[T,E]{
  ...
  .orderHash(by: OrderHashBy[imm E]): OrderHash[T] -> {'self
    .close -> this.close;
    .cmp a,b,m -> this.cmp(by,a,b,m);
...
OrderHashBy[T]:OrderBy[T]{
  ...
  .viewHash[AA](f:F[read AA,read Order[T]]):OrderByCmp[AA] -> {
    .cmp a,b,m -> f#a.cmp(f#a.close,f#b.close,m)};//This is the place we should point to.
    .hash a,h  -> f#a.hash(h);
    };
  }//but this is where we point.
//Can be obtained by: range of token from extra } to last group {
//expand this last group, reparenthesize. Recurse until no more last group {
OrderHashByCmp[T]:OrderHashBy[T]{
  .cmp[R](a:T,b:T,m:mut OrderMatch[R]): R;
...
Note: "{" cannot be used in single "#" interpolation expressions. Use at least "##".
